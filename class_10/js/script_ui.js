...
function init() {
    ...
    $("#section0 .link-wrap .link-unit .btn").mouseover(function(){
        var _index = $(this).parent().index(); //순서찾기
        // console.log(_index)
        $("#section0 .bg-wrap .bg").removeClass("on");
        $("#section0 .bg-wrap .bg").eq(_index).addClass("on"); //같은 순번의 배경 onclass추가
    });
    ...
    function checkPos(){
        var _tpos = $("#section2 .wave-wrap .unit").css("transform").split(",")[5]; //파도(unit)에 대한 transform값(matrix형식)중에서 6번째 값만 가져오기
        _tpos = _tpos.replace(")",""); //값 뒤의 ")"부분을 replace를 통해 제거
        var _th = $("#section2 .wave-wrap").height(); //백분률로 표현하기 위한 전체 높이값을 알아냄
        var _per = _tpos / _th; //현재값 / 전체값
        var _per1 = 100- Math.round(_per * 100); //나눈 값을 백분율로 표현 후 다시 100기준으로 반전
        // console.log(_per1);
